...
Everything under `.claude/`, `.opencode/`, and `.agents/` is **generated** from `.apm/` sources by APM. Direct edits will be overwritten on the next `apm install` run. The same applies to any scripts referenced by skills (e.g. `scripts/vcs-op`) — the canonical copy lives under `.apm/` (e.g. `.apm/skills/do/scripts/vcs-op`); copies elsewhere are build artifacts.
