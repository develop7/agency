...
applyTo: ".agents/**,.claude/**,.codex/**,.opencode/**"
---

## Generated Files — Do Not Edit Directly

> **This rule only applies if an `.apm/` directory exists somewhere in the project.** If there is no `.apm/` directory at any level, the host folders listed above are vendored directly and can be edited in place. To locate it, search for a directory matching `**/.apm/` from the project root.

Everything under `.agents/`, `.claude/`, `.codex/`, and `.opencode/` is **generated** from `.apm/` sources by APM. Direct edits will be overwritten on the next `apm install` run.
