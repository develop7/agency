...
- `branch` (default when `$ARGUMENTS` is empty) — audit only changes in the current branch/PR. Get the diff: use `.../skills/do/scripts/vcs-op diff-range <base>` to identify changed files and limit all subsequent steps to those files.
