...
- (side effect, unless `vcs_enabled` is `false`) one commit per Fix-in-this-PR finding, all pushed
...
- **Pre-compute diff scope.** Before spawning sub-agents, the main agent runs `vcs diff-against-base` and `vcs files-changed`. Pass the results to sub-agents as `diff_scope` and `changed_files` parameters. Sub-agents should not run VCS commands themselves.
- **Sub-agent prompts must be self-contained.** Sub-agents do not inherit context. Brief each one with: the full task prompt + relevant `research.plan` content (file paths, intended approach, key constraints) + the pre-computed `diff_scope` and `changed_files`. The sub-agent already knows to read its skill file; don't re-state the methodology.
...
- Every "Fix in this PR" finding has a corresponding commit on the feature branch (`vcs commits-since-base`), except when `vcs_enabled` is `false` (where fixes land in the working tree only).
