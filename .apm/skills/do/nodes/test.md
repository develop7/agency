...
- (side effect, unless `vcs_enabled` is `false`) one commit per test-driven fix, pushed
...
- Use `vcs files-changed` to identify changed files and determine which tests are relevant.
