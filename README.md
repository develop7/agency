...
- **`forge-pr`** — PR titles and descriptions devs actually want to read. Narrative paragraphs for the why; lists/tables/diagrams when the content is genuinely structured. GitHub today via the [`vcs-mcp`](https://github.com/develop7/vcs-toolkit-rs) MCP server (forked from [`vcs-toolkit-rs`](https://github.com/ZelAnton/vcs-toolkit-rs)); Bitbucket support is a config flip once upstream adds the `vcs-bitbucket` crate.
